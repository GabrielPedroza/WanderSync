...
func (uq *UserQuery) QueryLocation() *LocationQuery {
	query := (&LocationClient{config: uq.config}).Query()
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withLocation: uq.withLocation.Clone(),
...
func (uq *UserQuery) WithLocation(opts ...func(*LocationQuery)) *UserQuery {
	query := (&LocationClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withLocation = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withLocation; query != nil {
		if err := uq.loadLocation(ctx, query, nodes, nil,
...
func (uq *UserQuery) loadLocation(ctx context.Context, query *LocationQuery, nodes []*User, init func(*User), assign func(*User, *Location)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*User)
	for i := range nodes {
		if nodes[i].location_users == nil {
			continue
		}
		fk := *nodes[i].location_users
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "location_users" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
